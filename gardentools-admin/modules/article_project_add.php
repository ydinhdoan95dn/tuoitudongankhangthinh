...
if ($db->RowCount == 0)
	loadPageAdmin("Thể loại không tồn tại.", "?" . TTH_PATH . "=article_project_manager");
...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<a href="?<?= TTH_PATH ?>=article_project_list&id=<?= $article_project_menu_id ?>"><i class="fa fa-list"></i>
				<?= stripslashes($menu_name) ?></a>
		</li>
		<li>
			<i class="fa fa-plus-square-o"></i> Thêm dự án
		</li>
	</ol>
</div>
<!-- /.row -->
...
if ($typeFunc == 'add') {
	...
	elseif (empty($content))
		...
		if ($OK) {
			...
			// Finalize Article Project Gallery Manager data (bảng mới)
			$db->table = "article_project_gallery_category";
			$db->condition = "article_project_id = 0";
			$db->update(array('article_project_id' => $id_query));
			...
			loadPageSucces("Đã thêm Dự án thành công.", "?" . TTH_PATH . "=article_project_list&id=" . $article_project_menu_id);
			$OK = true;
		}
	}
} else {
...
if (!$OK) {
	...
	articleProjectStandalone("?" . TTH_PATH . "=article_project_add", "add", 0, $article_project_menu_id, $name, $title, $description, $keywords, $img, $img_note, $comment, $content, $is_active, $hot, $created_time, $upload_img_id, $error, $projectData);
}
?>