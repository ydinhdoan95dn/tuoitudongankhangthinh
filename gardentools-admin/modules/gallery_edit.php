...
if($typeFunc=='edit'){
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tên hình.</span>';
		...
		if($OK) {
			...
			$old_img_mobile = isset($_POST['img_mobile']) ? $_POST['img_mobile'] : '';
			...
			if($gallery_menu_id == 1 && $has_mobile_img == 1 && isset($_FILES['img_mobile_file']) && $_FILES['img_mobile_file']['size'] > 0) {
				...
				if($imgMobileUp->uploaded) {
					...
					// Xóa ảnh mobile cũ nếu có
					if($old_img_mobile != '' && $old_img_mobile != null) {
						$old_mobile_path = $dir_dest . DS . $old_img_mobile;
						if(file_exists($old_mobile_path)) {
							@unlink($old_mobile_path);
						}
					}
			...
			// Nếu bỏ check "Tải ảnh riêng cho Mobile" thì xóa ảnh mobile
			elseif($gallery_menu_id == 1 && $has_mobile_img == 0 && $old_img_mobile != '' && $old_img_mobile != null) {
				// Xóa file ảnh mobile
				$old_mobile_path = $dir_dest . DS . $old_img_mobile;
				if(file_exists($old_mobile_path)) {
					@unlink($old_mobile_path);
	...
	foreach($rows as $row) {
		...
		$name			    = $row['name'];
		$img                = $row['img'];
		$img_mobile         = isset($row['img_mobile']) ? $row['img_mobile'] : '';
	...
	// Xác định has_mobile_img từ img_mobile
	$has_mobile_img = ($img_mobile != '' && $img_mobile != null) ? 1 : 0;

	// Nếu gallery chưa có upload_id, tạo mới record trong uploads_tmp
	if($upload_img_id == 0) {
		...
		// Cập nhật upload_id cho gallery
		$db->table = "gallery";
		$data = array(
...
if(!$OK) gallery("?".TTH_PATH."=gallery_edit", "edit", $gallery_id, $gallery_menu_id, $name, $img, $comment, $content, $link, $is_active, $hot, $created_time, $upload_img_id, $error, $img_mobile, $has_mobile_img);
?>