...
<div class="row">
	<ol class="breadcrumb">
		...
		<li>
			<i class="fa fa-plus-square-o"></i> Thêm thể loại sản phẩm
		</li>
	</ol>
</div>
<!-- /.row -->
...
$OK = false;
$error = '';
// Khởi tạo biến article_project_menu_id từ POST hoặc default
$article_project_menu_id = isset($_POST['article_project_menu_id']) ? $_POST['article_project_menu_id']+0 : 0;
...
if($typeFunc=='add'){
	if(empty($name)) $error = '<span class="show-error">Vui lòng nhập tên thể loại.</span>';
		...
		if($OK) {
			...
			$data = array(
				...
				'parent'=>$parent_value,
				'article_project_menu_id'=>$article_project_menu_id, // Liên kết với dự án
				'sort'=>$sortOrder,
			...
			loadPageSucces("Đã thêm thể loại sản phẩm thành công.","?".TTH_PATH."=article_product_manager");
			$OK = true;
		}
	}
}
	...
	$home_image_type = "home_width";
	$location = "";
	$article_project_menu_id = 0;
}
if(!$OK) articleProductMenuStandalone("?".TTH_PATH."=article_product_menu_add", "add", 0, $name, $slug, $title, $description, $keywords, $parent, $comment, $font_icon, $is_active, $hot, $img, $error, $home_image_type, $location, $article_project_menu_id);
?>
