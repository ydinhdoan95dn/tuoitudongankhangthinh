<?php
/**
 * AJAX Project Gallery Handler
 * API endpoint quản lý thư viện ảnh dự án theo thư mục
 *
 * @version 1.0
 * @author DXMT
 */
...
function generateImageName($articleId, $tabType)
{
    return time() . '_' . $articleId . '_' . $tabType . '_' . md5(uniqid(mt_rand(), true));
}
...
{
    $articleId = intval(isset($_REQUEST['article_id']) ? $_REQUEST['article_id'] : 0);
    $tabType = isset($_REQUEST['tab_type']) ? $_REQUEST['tab_type'] : '';

    validateTabType($tabType, $validTabTypes);

    $db->table = "project_gallery_tab";
    $db->condition = "article_id = $articleId AND tab_type = '$tabType'";
    $db->limit = "1";
    $result = $db->select();
...
{
    $articleId = intval(isset($_POST['article_id']) ? $_POST['article_id'] : 0);
    ...
    if (is_array($existing) && !empty($existing[0])) {
        // Update
        $sql = "UPDATE {$prefix}project_gallery_tab SET
        ...
        // Insert
        $sql = "INSERT INTO {$prefix}project_gallery_tab (article_id, tab_type, description, show_description, created_at)
                VALUES ($articleId, '$tabType', '" . $db->clearText($description) . "', $showDescription, NOW())";
        $db->query($sql);
...
function listCategories($db, $validTabTypes)
{
    try {
        ...
        // Lấy danh sách category - $db->query() trả về array, không phải mysqli_result
        $sql = "SELECT c.*,
                (SELECT COUNT(*) FROM {$prefix}project_gallery_image WHERE category_id = c.id) as image_count
                FROM {$prefix}project_gallery_category c
                WHERE c.article_id = $articleId AND c.tab_type = '$tabType' AND c.is_active = 1
        ...
        $sql = "SELECT COUNT(*) as cnt FROM {$prefix}project_gallery_image
                WHERE article_id = $articleId AND tab_type = '$tabType' AND category_id IS NULL";
        $result = $db->query($sql);
        $rootCount = 0;
        if (is_array($result) && !empty($result[0])) {
...
function addCategory($db, $validTabTypes)
{
    ...
    // Kiểm tra trùng tên trong cùng article và tab
    $sql = "SELECT id FROM {$prefix}project_gallery_category
            WHERE article_id = $articleId AND tab_type = '$tabTypeEsc' AND name = '$nameEsc' AND is_active = 1";
    ...
    // Lấy sort max
    $sql = "SELECT MAX(sort) as max_sort FROM {$prefix}project_gallery_category
            WHERE article_id = $articleId AND tab_type = '$tabTypeEsc'";
    $result = $db->query($sql);
    $maxSort = 0;
    if (is_array($result) && !empty($result[0])) {
    ...
    // Insert
    $sql = "INSERT INTO {$prefix}project_gallery_category (article_id, tab_type, name, sort, created_at)
            VALUES ($articleId, '$tabTypeEsc', '$nameEsc', " . ($maxSort + 1) . ", NOW())";
    $db->query($sql);
...
function editCategory($db)
{
    ...
    $articleId = $existing[0]['article_id'];
    ...
    $tabTypeEsc = $db->clearText($tabType);
    $sql = "SELECT id FROM {$prefix}project_gallery_category
            WHERE article_id = $articleId AND tab_type = '$tabTypeEsc' AND name = '$nameEsc' AND id != $id AND is_active = 1";
    $duplicate = $db->query($sql);
    ...
    // Update - chỉ update name (không update updated_at vì cột có thể không tồn tại)
    $db->table = "project_gallery_category";
    $db->condition = "id = $id";
    $db->update(array('name' => $nameEsc));
...
function deleteCategory($db)
{
    ...
    // Xóa records ảnh
    $prefix = TTH_DATA_PREFIX;
    $sql = "DELETE FROM {$prefix}project_gallery_image WHERE category_id = $id";
    $db->query($sql);

    // Xóa category
    $sql = "DELETE FROM {$prefix}project_gallery_category WHERE id = $id";
    $db->query($sql);
...
function sortCategories($db)
{
    ...
    foreach ($ids as $sort => $id) {
        $id = intval($id);
        $sort = intval($sort);
        $sql = "UPDATE {$prefix}project_gallery_category SET sort = $sort WHERE id = $id";
        $db->query($sql);
    }
...
function listImages($db, $validTabTypes)
{
    $articleId = intval(isset($_REQUEST['article_id']) ? $_REQUEST['article_id'] : 0);
    $tabType = isset($_REQUEST['tab_type']) ? $_REQUEST['tab_type'] : '';
    // Xử lý category_id: chỉ là số nguyên > 0 mới được coi là valid, còn lại là NULL (ảnh chung/root)
    ...
    $condition = "article_id = $articleId AND tab_type = '$tabType'";
    if ($categoryId === null) {
    ...
    $sql = "SELECT * FROM {$prefix}project_gallery_image WHERE $condition ORDER BY sort ASC, id ASC";
    $result = $db->query($sql);
    ...
    jsonResponse(true, '', array('images' => $images));
}
...
function uploadImages($db, $validTabTypes)
{
    try {
        ...
        // Upload class is autoloaded from _F_CLASSES via Function.php autoloader
        ...
        $prefix = TTH_DATA_PREFIX;
        $tabTypeEsc = $db->clearText($tabType);
        $condition = "article_id = $articleId AND tab_type = '$tabTypeEsc'";
        if ($categoryId === null) {
            $condition .= " AND category_id IS NULL";
        } else {
            $condition .= " AND category_id = $categoryId";
        }
        $sql = "SELECT MAX(sort) as max_sort FROM {$prefix}project_gallery_image WHERE $condition";
        $result = $db->query($sql);
        $maxSort = 0;
        if (is_array($result) && !empty($result[0])) {
            $maxSort = (int) $result[0]['max_sort'];
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            $baseName = generateImageName($articleId, $tabType);
            ...
            $sql = "INSERT INTO {$prefix}project_gallery_image (article_id, tab_type, category_id, filename, title, sort, created_at)
                    VALUES ($articleId, '$tabTypeEsc', $catValue, '$filename', $titleValue, $maxSort, NOW())";
            $db->query($sql);
            $newId = $db->LastInsertID;
...
function updateImage($db)
{
    ...
    $sql = "UPDATE {$prefix}project_gallery_image SET title = '" . $db->clearText($title) . "' WHERE id = $id";
    $db->query($sql);
...
function deleteImage($db)
{
    ...
    // Xóa record
    $prefix = TTH_DATA_PREFIX;
    $sql = "DELETE FROM {$prefix}project_gallery_image WHERE id = $id";
    $db->query($sql);
...
function sortImages($db)
{
    ...
    foreach ($ids as $sort => $id) {
        $id = intval($id);
        $sort = intval($sort);
        $sql = "UPDATE {$prefix}project_gallery_image SET sort = $sort WHERE id = $id";
        $db->query($sql);
    }
...
function moveImage($db)
{
    ...
    $catValue = $categoryId === null ? 'NULL' : $categoryId;
    $sql = "UPDATE {$prefix}project_gallery_image SET category_id = $catValue WHERE id = $id";
    $db->query($sql);
...
function initTables($db)
{
    ...
    $prefix = TTH_DATA_PREFIX;
    $tables = array('project_gallery_tab', 'project_gallery_category', 'project_gallery_image');
    $existing = array();

    foreach ($tables as $table) {
        $fullTableName = $prefix . $table;
        // SHOW TABLES trả về array trong DatabaseConnect::query()
    ...
    // Table: project_gallery_tab
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}project_gallery_tab` (
    ...
    // Table: project_gallery_category
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}project_gallery_category` (
        `id` INT(11) NOT NULL AUTO_INCREMENT,
        `article_id` INT(11) NOT NULL DEFAULT 0,
        `tab_type` VARCHAR(50) NOT NULL,
    ...
    // Table: project_gallery_image
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}project_gallery_image` (
        `id` INT(11) NOT NULL AUTO_INCREMENT,
        `article_id` INT(11) NOT NULL DEFAULT 0,
        `tab_type` VARCHAR(50) NOT NULL,
    ...
    jsonResponse(true, 'Đã tạo các bảng thành công', array('created' => true));
}

function finalizeArticle($db)
{
    // Khi tạo article mới xong, cập nhật article_id từ 0 sang ID thực
    $tempId = intval(isset($_POST['temp_id']) ? $_POST['temp_id'] : 0);
    $articleId = intval(isset($_POST['article_id']) ? $_POST['article_id'] : 0);
    $prefix = TTH_DATA_PREFIX;

    if (!$articleId) {
        jsonResponse(false, 'Article ID không hợp lệ');
    }
    ...
    foreach ($tables as $table) {
        $sql = "UPDATE {$prefix}{$table} SET article_id = $articleId WHERE article_id = $tempId";
        $db->query($sql);
    }

    jsonResponse(true, 'Đã cập nhật article_id');
}
