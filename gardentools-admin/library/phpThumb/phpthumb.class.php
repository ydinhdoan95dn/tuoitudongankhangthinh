...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	function phpThumbDebug($level='') {
		...
		$DebugOutput[] = 'get_magic_quotes_gpc()         = '.$this->phpThumbDebugVarDump(version_compare(PHP_VERSION, '8.0.0', '<') && function_exists('get_magic_quotes_gpc') ? @get_magic_quotes_gpc() : false);
		$DebugOutput[] = 'get_magic_quotes_runtime()     = '.$this->phpThumbDebugVarDump(version_compare(PHP_VERSION, '8.0.0', '<') && function_exists('get_magic_quotes_runtime') ? @get_magic_quotes_runtime() : false);
