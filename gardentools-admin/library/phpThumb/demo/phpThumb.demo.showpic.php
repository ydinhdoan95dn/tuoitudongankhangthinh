...
function SafeStripSlashes($string) {
	// PHP 8.0+: get_magic_quotes_gpc() removed
	$magic_quotes = version_compare(PHP_VERSION, '8.0.0', '<') && function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc();
	return ($magic_quotes ? stripslashes($string) : $string);
}
