 ...
 * Upload video riêng biệt cho sản phẩm BĐS, trả về tên file để lưu vào form
...
function handleUpload()
{
    ...
    // Create upload directory for product video
    $uploadDir = dirname(dirname(dirname(__FILE__))) . '/uploads/product/video';
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0755, true);
    }

    // Generate unique filename
    $articleProductId = isset($_POST['article_product_id']) ? intval($_POST['article_product_id']) : 0;
    $timestamp = time();
    $randomStr = substr(md5(uniqid(mt_rand(), true)), 0, 8);
    $newFilename = 'pvid_' . $randomStr . '_' . $timestamp . ($articleProductId > 0 ? '_' . $articleProductId : '') . '.' . $extension;
    ...
    if (move_uploaded_file($file['tmp_name'], $targetPath)) {
        echo json_encode(array(
            ...
            'url' => '/uploads/product/video/' . $newFilename,
            'size' => $file['size'],
...
function handleDelete()
{
    ...
    $filePath = dirname(dirname(dirname(__FILE__))) . '/uploads/product/video/' . $filename;
