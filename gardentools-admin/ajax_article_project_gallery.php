<?php
/**
 * AJAX Article Project Gallery Handler
 * API endpoint quản lý thư viện ảnh dự án BĐS (bảng dxmt_article_project)
 *
 * @version 2.0
 * @author DXMT
 */
...
function generateImageName($articleProjectId, $tabType)
{
    return time() . '_ap' . $articleProjectId . '_' . $tabType . '_' . md5(uniqid(mt_rand(), true));
}
...
{
    $articleProjectId = intval(isset($_REQUEST['article_project_id']) ? $_REQUEST['article_project_id'] : 0);
    $tabType = isset($_REQUEST['tab_type']) ? $_REQUEST['tab_type'] : '';

    validateTabType($tabType, $validTabTypes);

    $db->table = "article_project_gallery_tab";
    $db->condition = "article_project_id = $articleProjectId AND tab_type = '$tabType'";
    $db->limit = "1";
    $result = $db->select();
...
{
    $articleProjectId = intval(isset($_POST['article_project_id']) ? $_POST['article_project_id'] : 0);
    ...
    if (is_array($existing) && !empty($existing[0])) {
        // Update
        $sql = "UPDATE {$prefix}article_project_gallery_tab SET
        ...
        // Insert
        $sql = "INSERT INTO {$prefix}article_project_gallery_tab (article_project_id, tab_type, description, show_description, created_at)
                VALUES ($articleProjectId, '$tabType', '" . $db->clearText($description) . "', $showDescription, NOW())";
        $db->query($sql);
...
function listCategories($db, $validTabTypes)
{
    try {
        ...
        $sql = "SELECT c.*,
                (SELECT COUNT(*) FROM {$prefix}article_project_gallery_image WHERE category_id = c.id) as image_count
                FROM {$prefix}article_project_gallery_category c
                WHERE c.article_project_id = $articleProjectId AND c.tab_type = '$tabType' AND c.is_active = 1
        ...
        $sql = "SELECT COUNT(*) as cnt FROM {$prefix}article_project_gallery_image
                WHERE article_project_id = $articleProjectId AND tab_type = '$tabType' AND category_id IS NULL";
        $result = $db->query($sql);
        $rootCount = 0;
        if (is_array($result) && !empty($result[0])) {
...
function addCategory($db, $validTabTypes)
{
    ...
    // Kiểm tra trùng tên trong cùng article_project và tab
    $sql = "SELECT id FROM {$prefix}article_project_gallery_category
            WHERE article_project_id = $articleProjectId AND tab_type = '$tabTypeEsc' AND name = '$nameEsc' AND is_active = 1";
    ...
    // Lấy sort max
    $sql = "SELECT MAX(sort) as max_sort FROM {$prefix}article_project_gallery_category
            WHERE article_project_id = $articleProjectId AND tab_type = '$tabTypeEsc'";
    $result = $db->query($sql);
    $maxSort = 0;
    if (is_array($result) && !empty($result[0])) {
    ...
    // Insert
    $sql = "INSERT INTO {$prefix}article_project_gallery_category (article_project_id, tab_type, name, sort, created_at)
            VALUES ($articleProjectId, '$tabTypeEsc', '$nameEsc', " . ($maxSort + 1) . ", NOW())";
    $db->query($sql);
...
function editCategory($db)
{
    ...
    $db->table = "article_project_gallery_category";
    $db->condition = "id = $id";
    $db->order = "";
    $db->limit = "1";
    $existing = $db->select();
    ...
    $articleProjectId = $existing[0]['article_project_id'];
    ...
    $tabTypeEsc = $db->clearText($tabType);
    $sql = "SELECT id FROM {$prefix}article_project_gallery_category
            WHERE article_project_id = $articleProjectId AND tab_type = '$tabTypeEsc' AND name = '$nameEsc' AND id != $id AND is_active = 1";
    $duplicate = $db->query($sql);
...
function deleteCategory($db)
{
    ...
    $db->table = "article_project_gallery_image";
    ...
    // Xóa records ảnh
    $prefix = TTH_DATA_PREFIX;
    $sql = "DELETE FROM {$prefix}article_project_gallery_image WHERE category_id = $id";
    $db->query($sql);

    // Xóa category
    $sql = "DELETE FROM {$prefix}article_project_gallery_category WHERE id = $id";
    $db->query($sql);
...
function sortCategories($db)
{
    ...
    foreach ($ids as $sort => $id) {
        $id = intval($id);
        $sort = intval($sort);
        $sql = "UPDATE {$prefix}article_project_gallery_category SET sort = $sort WHERE id = $id";
        $db->query($sql);
    }
...
function listImages($db, $validTabTypes)
{
    ...
    $condition = "article_project_id = $articleProjectId AND tab_type = '$tabType'";
    if ($categoryId === null) {
    ...
    $sql = "SELECT * FROM {$prefix}article_project_gallery_image WHERE $condition ORDER BY sort ASC, id ASC";
    $result = $db->query($sql);
...
function uploadImages($db, $validTabTypes)
{
    try {
        ...
        $prefix = TTH_DATA_PREFIX;
        $tabTypeEsc = $db->clearText($tabType);
        $condition = "article_project_id = $articleProjectId AND tab_type = '$tabTypeEsc'";
        if ($categoryId === null) {
            $condition .= " AND category_id IS NULL";
        } else {
            $condition .= " AND category_id = $categoryId";
        }
        $sql = "SELECT MAX(sort) as max_sort FROM {$prefix}article_project_gallery_image WHERE $condition";
        $result = $db->query($sql);
        $maxSort = 0;
        if (is_array($result) && !empty($result[0])) {
            $maxSort = (int) $result[0]['max_sort'];
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            $baseName = generateImageName($articleProjectId, $tabType);
            ...
            $sql = "INSERT INTO {$prefix}article_project_gallery_image (article_project_id, tab_type, category_id, filename, title, sort, created_at)
                    VALUES ($articleProjectId, '$tabTypeEsc', $catValue, '$filename', $titleValue, $maxSort, NOW())";
            $db->query($sql);
            $newId = $db->LastInsertID;
...
function updateImage($db)
{
    ...
    $sql = "UPDATE {$prefix}article_project_gallery_image SET title = '" . $db->clearText($title) . "' WHERE id = $id";
    $db->query($sql);
...
function deleteImage($db)
{
    ...
    // Xóa record
    $prefix = TTH_DATA_PREFIX;
    $sql = "DELETE FROM {$prefix}article_project_gallery_image WHERE id = $id";
    $db->query($sql);
...
function sortImages($db)
{
    ...
    foreach ($ids as $sort => $id) {
        $id = intval($id);
        $sort = intval($sort);
        $sql = "UPDATE {$prefix}article_project_gallery_image SET sort = $sort WHERE id = $id";
        $db->query($sql);
    }
...
function moveImage($db)
{
    ...
    $catValue = $categoryId === null ? 'NULL' : $categoryId;
    $sql = "UPDATE {$prefix}article_project_gallery_image SET category_id = $catValue WHERE id = $id";
    $db->query($sql);
...
{
    $prefix = TTH_DATA_PREFIX;
    $tables = array('article_project_gallery_tab', 'article_project_gallery_category', 'article_project_gallery_image');
    $existing = array();
    ...
    // Table: article_project_gallery_tab
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}article_project_gallery_tab` (
        `id` INT(11) NOT NULL AUTO_INCREMENT,
        `article_project_id` INT(11) NOT NULL DEFAULT 0,
        ...
        PRIMARY KEY (`id`),
        KEY `article_project_id` (`article_project_id`),
        KEY `tab_type` (`tab_type`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci";

    // Table: article_project_gallery_category
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}article_project_gallery_category` (
        `id` INT(11) NOT NULL AUTO_INCREMENT,
        `article_project_id` INT(11) NOT NULL DEFAULT 0,
        `tab_type` VARCHAR(50) NOT NULL,
    ...
    // Table: article_project_gallery_image
    $createStatements[] = "CREATE TABLE IF NOT EXISTS `{$prefix}article_project_gallery_image` (
        `id` INT(11) NOT NULL AUTO_INCREMENT,
        `article_project_id` INT(11) NOT NULL DEFAULT 0,
        `tab_type` VARCHAR(50) NOT NULL,
...
{
    // Khi tạo article_project mới xong, cập nhật article_project_id từ 0 sang ID thực
    $tempId = intval(isset($_POST['temp_id']) ? $_POST['temp_id'] : 0);
    $articleProjectId = intval(isset($_POST['article_project_id']) ? $_POST['article_project_id'] : 0);
    $prefix = TTH_DATA_PREFIX;

    if (!$articleProjectId) {
        jsonResponse(false, 'Article Project ID không hợp lệ');
    }
    ...
    foreach ($tables as $table) {
        $sql = "UPDATE {$prefix}{$table} SET article_project_id = $articleProjectId WHERE article_project_id = $tempId";
        $db->query($sql);
    }

    jsonResponse(true, 'Đã cập nhật article_project_id');
}
