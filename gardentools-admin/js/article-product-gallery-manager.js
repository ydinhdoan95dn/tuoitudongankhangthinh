/**
 * ARTICLE PRODUCT GALLERY MANAGER
 * Quản lý thư viện ảnh sản phẩm BĐS (bảng dxmt_article_product)
 *
 * @version 2.0
 * @author DXMT
 */
...
(function() {
    ...
    const API_URL = ADMIN_PATH + 'ajax_article_product_gallery.php';

    class ArticleProductGalleryManager {
        constructor(container, options = {}) {
            ...
            if (!this.container) {
                console.error('[APDGM] Container not found');
                return;
            }
        ...
        async init() {
            console.log('[APDGM] Initializing...', this.options);
        ...
        async initTables() {
            try {
                ...
                if (response.success) {
                    console.log('[APDGM] Tables ready');
                }
            } catch (e) {
                console.error('[APDGM] Init tables error:', e);
            }
        }
            ...
            this.container.innerHTML = `
                <div class="pgm-container" data-tab="${this.options.tabType}">
                    ...
                    <div class="pgm-toolbar">
                        <div class="pgm-toolbar-left"></div>
                ...
                <div class="pgm-modal-overlay pgm-pending-modal" id="pgm-pending-modal-${this.options.tabType}">
                    <div class="pgm-modal pgm-pending-modal-content">
                        ...
                        <div class="pgm-modal-body pgm-pending-body">
                            ...
                            <div class="pgm-pending-queue" id="pgm-pending-queue-${this.options.tabType}"></div>
        ...
        async loadData() {
            ...
            try {
                ...
                console.error('[APDGM] Load data error:', e);
        ...
        async loadTabDescription() {
            const response = await this.api('get_tab', {
                article_product_id: this.options.articleProductId,
        ...
        async loadImages(categoryId) {
            ...
            try {
                ...
                console.error('[APDGM] Load images error:', e);
        ...
        async saveNewCategory() {
            ...
            try {
                ...
                console.error('[APDGM] Add category error:', e);
        ...
        startEditCategory(categoryId) {
            ...
            const handleSave = () => this.finishEditCategory(categoryId);
            const handleCancel = () => this.cancelEditCategory(categoryId);
        ...
        async finishEditCategory(categoryId) {
            ...
            try {
                ...
                console.error('[APDGM] Edit category error:', e);
        ...
        async deleteCategory(categoryId) {
            ...
            try {
                ...
                console.error('[APDGM] Delete category error:', e);
                catElement?.classList.remove('pgm-category-deleting');
        ...
        async uploadPendingQueue() {
            ...
            formData.append('article_product_id', this.options.articleProductId);
            ...
            try {
                ...
                console.error('[APDGM] Upload error:', e);
                this.hideFullscreenLoading();
        ...
        async saveImageTitle() {
            ...
            try {
                ...
                console.error('[APDGM] Save image title error:', e);
                this.showToast('Lỗi lưu tiêu đề', 'error');
            }
        }

        async deleteImage(imageId, skipConfirm = false) {
            console.log('[APDGM] deleteImage called with imageId:', imageId);
            ...
            try {
                ...
                console.error('[APDGM] Delete image error:', e);
        ...
        async saveTabDescription() {
            ...
            try {
                ...
                console.error('[APDGM] Save tab description error:', e);
        ...
        setArticleProductId(articleProductId) {
            this.options.articleProductId = articleProductId;
        }

        async finalizeArticle(tempId, articleProductId) {
            return this.api('finalize_article', {
                temp_id: tempId,
                article_product_id: articleProductId
            });
        }
    ...
    // Export to global
    window.ArticleProductGalleryManager = ArticleProductGalleryManager;
