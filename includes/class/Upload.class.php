    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                // PHP 8.2+ compatible: dùng mb_convert_encoding thay utf8_encode/utf8_decode
                $from_chars = 'ŠŽšžŸÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÑÒÓÔÕÖØÙÚÛÜÝàáâãäåçèéêëìíîïñòóôõöøùúûüýÿ';
                $to_chars = 'SZszYAAAAAACEEEEIIIINOOOOOOUUUUYaaaaaaceeeeiiiinoooooouuuuyy';
                if (function_exists('mb_convert_encoding')) {
                    $name_latin1 = mb_convert_encoding($this->file_dst_name_body, 'ISO-8859-1', 'UTF-8');
                    $from_latin1 = mb_convert_encoding($from_chars, 'ISO-8859-1', 'UTF-8');
                    $this->file_dst_name_body = mb_convert_encoding(strtr($name_latin1, $from_latin1, $to_chars), 'UTF-8', 'ISO-8859-1');
                } else {
                    // Fallback cho các ký tự đặc biệt
